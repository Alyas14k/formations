...
<body>
        ...
        Chevalier de l'ordre du mérite <br>  national
         ...
         <div><img style="width: 130px; margin: left 30px; margin-top:auto;" src="{{asset('theme/img/logo.png')}}" alt="Logo FORMATIC"></div> 
    ...
    <Center><h2 style="color: #5326B6; margin-top:50px;"><u>FORMATIC</u></h2></Center>
    <Center><h4 style="margin-top: -10px"><strong> Formation Soucrite: {{getformation($inscrit->formation_id)['titre']}}</strong></h4>
    </Center>
        
    </div>       
    <!-- <p style="background-color: red;">Du 20 au 28 Aôut à Bobo Dioulasso </p>  -->
        
           <center> <p style="color: red;"><u> Fiche de Paiement N°:  {{$last->code}}</u> </p></center>
        ...
        <table class="paiement">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ date("d-m-Y à H:i", strtotime($paiement->created_at)) }}</td>
                </tr>   
                @php
                ...
                @endforeach
                <tr>
                    <td colspan="4" style="text-align: left; font-weight: bold; background-color: #5326B6; color: #fff;">Total Payé</td>
                    <td style="font-weight: bold; background-color: #5326B6; color: #fff;">{{ $totalPaye }}</td>
                </tr>
            </tbody>
        </table>
        @endif
...
<p>Directeur</p>
    <!-- <img style="width: 90px; height: 30px;" src="{{asset('img/CERFI.PNG')}}" alt="Cachet"></div>  -->
        ...
        <i><p style="font-size: 13px;">NB: Cette fiche est strictement personnelle, elle vous sera demandée lors de la formation </p></i>
       <p style="font-size: 13px;"> Pour toute information contactez nous au : <span style="font-weight: bold;">+226 57 47 67 59 - 65 60 29 03 - 60 71 96 34</span></p></i><br>
        <span style="width: 40%; float: right;">Fait à Ouagadougou le <span style="font-weight: bold;">{{ date("d-m-Y à H:i", strtotime($last->created_at)) }}</span> </span> <br>
